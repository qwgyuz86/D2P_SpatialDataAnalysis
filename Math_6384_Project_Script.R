...
## Set up the Geary Test parameters
## Using magicfor to store test result
magic_for(silent = TRUE, progress = TRUE)
for (i in ETH) {
  case = acs19[[i]] # number of cases
  r = sum(case)/sum(pop) # estimated risk (global)
  rates = case/pop # rates for each region
  e = r * pop # expected per region
  print(paste0("For ", i))
  print(paste0("Obs count: ", case, ". Exp count: ", ceiling(e)))
  ...
  print(paste0("Observed Geary's C statistic for ", i))
  ...
  tsimb = numeric(nsim)
  for (j in 1:nsim) {
    tsimb[j] = geary(rpois(N, e)/pop, listw = lw, n = N, n1 = N - 1, S0 = Szero(lw))$C
  }
  
  # p-value
  print(paste0("P-value for Geary's C statistic for ", i))
  p_value = (sum(tsimb <= t0b) + 1)/(nsim + 1)
  print(p_value)
